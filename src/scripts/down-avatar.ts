...
async function main() {
    ...
    let startId = 0;
    if (fs.existsSync(path.join(basePath, 'c.txt'))) {
        startId = parseInt(fs.readFileSync(path.join(basePath, 'c.txt'), 'utf-8'));
    }
    ...
    for (let i = startId; i < 280000; i++) {
        ...
        if (promises.length >= BATCH_SIZE) {
            ...
            promises.length = 0;
            fs.writeFileSync(path.join(basePath, 'c.txt'), i.toString());
        }
